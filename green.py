...
# Массовые действия (посадка/сбор урожая сразу на всех грядках) открываются
# только тогда, когда открыто хотя бы столько грядок — до этого кнопки в
# меню сада ВИДНЫ, но нажатие на них даёт алерт с ошибкой (см. mass_plant/
# mass_harvest ниже и хендлеры garden_massplant*/garden_massharvest в main.py).
MASS_ACTIONS_MIN_PLOTS = 6
...
# Инвентарь и котёл слияния: сначала выбор тира, а внутри тира — страницы,
# иначе при 220 видах цветов список получается нечитаемо длинным.
INV_PER_PAGE = 8
MERGE_PER_PAGE = 8
...
def ensure_garden(data: dict) -> dict:
    ...
    # Текущий выбор для МАССОВОЙ посадки — список видов семян, которые
    # подсвечены зелёным/success в меню и будут посажены одновременно по
    # кнопке "Посадить" (см. mass_plant_toggle_pick/mass_plant_confirm).
    picks = g.setdefault("mass_plant_pick", [])
...
# ── Массовые действия: посадка и сбор урожая сразу на ВСЕХ грядках.
# Открываются только при MASS_ACTIONS_MIN_PLOTS открытых грядках — см.
# mass_actions_unlocked. До этого функции ниже возвращают {"ok": False,
# "reason": "locked", ...}, а кнопки в меню сада (garden_keyboard) остаются
# видимыми, но нажатие на них даёт хендлеру main.py повод показать алерт. ──

def mass_actions_unlocked(g: dict) -> bool:
    return g.get("plot_count", 0) >= MASS_ACTIONS_MIN_PLOTS


def has_empty_plots(g: dict) -> bool:
    return any(p is None for p in g["plots"][:g["plot_count"]])


def _empty_plot_indices(g: dict) -> list[int]:
    return [idx for idx in range(g["plot_count"]) if g["plots"][idx] is None]


def _ready_plot_indices(g: dict) -> list[int]:
    idxs = []
    for idx, plot in enumerate(g["plots"]):
        if plot is None:
            continue
        stage, _, _, _ = plot_state(plot, _plot_level_mult(g, idx))
        if stage == "ready":
            idxs.append(idx)
    return idxs


def mass_harvest(data: dict) -> dict:
    """Собирает урожай сразу со ВСЕХ готовых грядок. Каждая грядка проходит
    через обычный harvest_plot (со всеми её бонусами/джекпотами/открытиями
    коллекции), результаты просто суммируются."""
    g = ensure_garden(data)
    if not mass_actions_unlocked(g):
        return {"ok": False, "reason": "locked", "need": MASS_ACTIONS_MIN_PLOTS, "have": g["plot_count"]}

    ready = _ready_plot_indices(g)
    if not ready:
    ...
    flowers = []
    for idx in ready:
        res = harvest_plot(data, idx)
        if not res["ok"]:
            continue
        total_essence += res["essence"]
        total_xp += res["xp"]
        flowers.append(res["flower"])
        if res.get("jackpot"):
            jackpots += 1
        if res.get("grand_bloom"):
            grand_bloom = True
...
def mass_plant_toggle_pick(data: dict, flower_key: str) -> dict:
    """Переключает выбор вида для массовой посадки — тап по цветку в меню не
    сажает сразу, а добавляет/убирает его из списка выбранных (можно выбрать
    НЕСКОЛЬКО разных видов одновременно, каждый подсвечивается зелёным/success).
    Сама посадка происходит позже, по кнопке «Посадить» (см. mass_plant_confirm).
    Если пыльцы/семян не хватает даже на один экземпляр — выбрать нельзя вовсе
    (хендлер в main.py не даёт нажать такую кнопку, см. garden_noop)."""
    g = ensure_garden(data)
    if not mass_actions_unlocked(g):
    ...
    picks = g.setdefault("mass_plant_pick", [])
    if flower_key in picks:
        picks.remove(flower_key)
        return {"ok": True, "picked": False, "picks": list(picks)}

    picks.append(flower_key)
    return {"ok": True, "picked": True, "picks": list(picks)}


def mass_plant_confirm(data: dict) -> dict:
    """Сажает ВСЕ выбранные виды (mass_plant_pick) сразу — по кнопке
    «🌱 Посадить». Свободные грядки заполняются по очереди (round-robin) между
    выбранными видами: 1-я грядка — первый выбранный вид, 2-я — второй, и
    так по кругу, пока не кончатся либо свободные грядки, либо ресурсы
    (пыльца/семена) у ВСЕХ выбранных видов разом."""
    g = ensure_garden(data)
    if not mass_actions_unlocked(g):
        return {"ok": False, "reason": "locked", "need": MASS_ACTIONS_MIN_PLOTS, "have": g["plot_count"]}

    picks = list(g.get("mass_plant_pick") or [])
    if not picks:
        return {"ok": False, "reason": "no_pick"}

    empty = _empty_plot_indices(g)
    if not empty:
    ...
    ran_out = False
    for idx in empty:
        if len(exhausted) >= len(picks):
            ran_out = True
            break
        planted_here = False
        for _ in range(len(picks)):
            key = picks[cycle % len(picks)]
            cycle += 1
            if key in exhausted:
                continue
            result = plant_flower(data, idx, key)
            if result["ok"]:
    ...
    g["mass_plant_pick"] = []
    flowers = {key: FLOWERS_BY_KEY[key] for key in counts}
    return {"ok": True, "planted": total, "counts": counts, "flowers": flowers, "stop_reason": "exhausted" if ran_out else None}


def mass_plant(data: dict, flower_key: str) -> dict:
    """Сажает ОДИН выбранный вид семени сразу во ВСЕ свободные грядки —
    останавливается сама, если закончилась мистическая пыльца (для семян
    тира 1) или запас семян в инвентаре (для тира 2+), не трогая при этом
    уже засаженные в рамках этого же вызова грядки."""
    g = ensure_garden(data)
    if not mass_actions_unlocked(g):
        return {"ok": False, "reason": "locked", "need": MASS_ACTIONS_MIN_PLOTS, "have": g["plot_count"]}
    ...
    last_flower = None
    for idx in empty:
        result = plant_flower(data, idx, flower_key)
        if not result["ok"]:
            stop_reason = result["reason"]
            break
        planted += 1
        last_flower = result["flower"]

    if planted == 0:
        return {"ok": False, "reason": stop_reason or "unknown_flower"}

    return {"ok": True, "planted": planted, "flower": last_flower, "stop_reason": stop_reason}
...
def garden_keyboard(data: dict, page: int = 0):
    ...
    # Массовые действия: кнопки видны ВСЕГДА, но пока не открыто
    # MASS_ACTIONS_MIN_PLOTS грядок — подписаны замком, а нажатие на них
...
# ── Меню выбора семени для МАССОВОЙ посадки — по структуре повторяет
# plant_menu_*/plant_inventory_*, но без привязки к одной грядке: выбранный
# вид сажается сразу во все свободные грядки (см. mass_plant в хендлере
# garden_massplantdo в main.py). ──

def mass_plant_menu_text(data: dict, page: int = 0) -> str:
    g = ensure_garden(data)
    ...
    lines = [
        ...
        f'<blockquote><i>Выбери один или несколько видов — они подсветятся зелёным. '
        f'Нажми «Посадить», чтобы разом засадить ВСЕ свободные грядки ({empty} шт.), '
        f'по очереди чередуя выбранные виды. Обычные семена покупаются за '
        f'{ESSENCE_NAME.lower()} — посадка остановится сама, если пыльца/семена '
        f'закончатся раньше грядок.</i></blockquote>',
        f'📄 Страница: <b>{page + 1}/{total_pages}</b>',
    ]
    pick = g.get("mass_plant_pick") or []
    if pick:
        names = ", ".join(flower_label(FLOWERS_BY_KEY[k]) for k in pick if k in FLOWERS_BY_KEY)
        lines.append(f'\n✅ <b>Выбрано ({len(pick)}):</b> {names}')
    return "\n".join(lines)
...
def mass_plant_menu_keyboard(data: dict, page: int = 0):
    ...
    if pick:
        b.row(InlineKeyboardButton(text=f'🌱 Посадить ({len(pick)} выбрано)',
    ...
    seed_keys = [k for k, cnt in g["inventory"].items() if cnt > 0 and FLOWERS_BY_KEY[k]["tier"] > 1]
    if seed_keys:
        b.row(InlineKeyboardButton(text="🎒 Засадить семенем из инвентаря", callback_data="garden_massplantinv:0"))

    b.row(InlineKeyboardButton(text="Назад", icon_custom_emoji_id=BACK_ICON_EMOJI_ID, callback_data="garden"))
    return b.as_markup()


def mass_plant_inventory_text(data: dict, page: int = 0) -> str:
    g = ensure_garden(data)
    empty = len(_empty_plot_indices(g))
    seeds = [(k, c) for k, c in g["inventory"].items() if c > 0 and FLOWERS_BY_KEY[k]["tier"] > 1]
    total_pages = _seed_pages(len(seeds)) if seeds else 1
    ...
    if not seeds:
        ...
        body = (f'<i>Выбери один или несколько семян — они подсветятся зелёным. Нажми «Посадить», '
                f'чтобы разом засадить ими все свободные грядки ({empty} шт.), чередуя выбранные виды '
                f'по очереди, пока не закончатся либо семена, либо грядки.</i>')
    lines = [
    ...
    pick = g.get("mass_plant_pick") or []
    picked_here = [k for k in pick if k in dict(seeds)]
    if picked_here:
        names = ", ".join(flower_label(FLOWERS_BY_KEY[k]) for k in picked_here)
        lines.append(f'\n✅ <b>Выбрано ({len(pick)} всего):</b> {names}')
    return "\n".join(lines)
...
def _inv_pages(count: int) -> int:
    return max(1, (count + INV_PER_PAGE - 1) // INV_PER_PAGE)


def _inv_tier_items(data: dict, tier: int) -> list:
    g = ensure_garden(data)
    return sorted(
        [(k, c) for k, c in g["inventory"].items() if FLOWERS_BY_KEY[k]["tier"] == tier and c > 0],
        key=lambda kv: FLOWERS_BY_KEY[kv[0]]["name"],
    )


def inventory_menu_text(data: dict) -> str:
    g = ensure_garden(data)
    ...
    return (
        '🎒 <b>Инвентарь сада</b>\n'
        '<blockquote><i>Выбери тир, чтобы посмотреть собранные цветки.</i></blockquote>'
    )


def inventory_menu_keyboard(data: dict):
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    from aiogram.types import InlineKeyboardButton
    ...
    b = InlineKeyboardBuilder()
    for tier in range(TIER_MIN, TIER_MAX + 1):
        tier_items = _inv_tier_items(data, tier)
        if not tier_items:
            continue
        total = sum(c for _, c in tier_items)
        b.row(InlineKeyboardButton(
            text=f'{TIER_ICON[tier]} {TIER_NAMES[tier]} — {len(tier_items)} вид. · {total} шт.',
            callback_data=f"garden_invtier:{tier}:0",
        ))
    b.row(InlineKeyboardButton(text="Назад", icon_custom_emoji_id=BACK_ICON_EMOJI_ID, callback_data="garden"))
    return b.as_markup()


def inventory_tier_text(data: dict, tier: int, page: int = 0) -> str:
    tier_items = _inv_tier_items(data, tier)
    pages = _inv_pages(len(tier_items)) if tier_items else 1
    page = max(0, min(pages - 1, page))
    total = sum(c for _, c in tier_items)
    lines = [
        f'🎒 <b>Инвентарь — {TIER_ICON[tier]} {TIER_NAMES[tier]}</b>',
        f'<blockquote><i>Всего в этом тире: {len(tier_items)} вид., {total} шт.</i></blockquote>',
        f'📄 Страница {page + 1}/{pages}',
    ]
    return "\n".join(lines)


def inventory_tier_keyboard(data: dict, tier: int, page: int = 0):
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    from aiogram.types import InlineKeyboardButton
    ...
    pages = _inv_pages(len(tier_items)) if tier_items else 1
    page = max(0, min(pages - 1, page))
    start = page * INV_PER_PAGE

    b = InlineKeyboardBuilder()
    for key, cnt in tier_items[start:start + INV_PER_PAGE]:
        f = FLOWERS_BY_KEY[key]
        b.row(InlineKeyboardButton(
            text=f'{flower_label(f)} ×{cnt}',
            callback_data=f"garden_flower:{key}:{page}",
        ))

    if tier_items:
        nav = []
        if page > 0:
            nav.append(InlineKeyboardButton(text="◀️", callback_data=f"garden_invtier:{tier}:{page - 1}"))
...
def flower_detail_keyboard(data: dict, flower_key: str, page: int = 0):
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    from aiogram.types import InlineKeyboardButton
    ...
    if cnt >= 1:
        b.row(
            InlineKeyboardButton(text="💰 Продать 1", callback_data=f"garden_sell:{flower_key}:1:{page}"),
            InlineKeyboardButton(text="💰 Продать всё", callback_data=f"garden_sell:{flower_key}:all:{page}"),
        )
    b.row(InlineKeyboardButton(
        text="Назад", icon_custom_emoji_id=BACK_ICON_EMOJI_ID,
        callback_data=f"garden_invtier:{tier}:{page}",
    ))
    return b.as_markup()
...
def _merge_pages(count: int) -> int:
    return max(1, (count + MERGE_PER_PAGE - 1) // MERGE_PER_PAGE)


def _merge_tier_flowers(data: dict, tier: int) -> list:
    """Цветки этого тира, доступные для показа в котле — те, что есть
    в инвентаре и/или уже лежат в котле."""
    g = ensure_garden(data)
    cart = g["merge_cart"]
    result = []
...
def merge_tier_text(data: dict, tier: int, page: int = 0) -> str:
    g = ensure_garden(data)
    cart = g["merge_cart"]
    tier_flowers = _merge_tier_flowers(data, tier)
    pages = _merge_pages(len(tier_flowers)) if tier_flowers else 1
    page = max(0, min(pages - 1, page))
    lines = [
...
def merge_tier_keyboard(data: dict, tier: int, page: int = 0):
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    from aiogram.types import InlineKeyboardButton
    ...
    pages = _merge_pages(len(tier_flowers)) if tier_flowers else 1
    page = max(0, min(pages - 1, page))
    start = page * MERGE_PER_PAGE

    b = InlineKeyboardBuilder()
    for f in tier_flowers[start:start + MERGE_PER_PAGE]:
        in_cart = cart["items"].get(f["key"], 0) if cart.get("tier") == tier else 0
        ...
        b.row(InlineKeyboardButton(
            text=label,
            callback_data=f"garden_mergeadd:{f['key']}:{page}" if avail > 0 else "garden_noop",
        ))

    if tier_flowers:
        nav = []
        if page > 0:
            nav.append(InlineKeyboardButton(text="◀️", callback_data=f"garden_mergetier:{tier}:{page - 1}"))
        nav.append(InlineKeyboardButton(text=f"· {page + 1}/{pages} ·", callback_data="garden_noop"))
        if page < pages - 1:
            nav.append(InlineKeyboardButton(text="▶️", callback_data=f"garden_mergetier:{tier}:{page + 1}"))
        b.row(*nav)
