...
async def cmd_city_buy(message: Message):
    ...
    if not await aio_try_spend_balance(u["user_id"], total):
        await message.reply(
            f"💸 Недостаточно {CURRENCY_NAME} для этой покупки.",
            parse_mode="HTML",
        )
        return
    if not await aio_try_adjust_inventory(u["user_id"], item, qty):
        await aio_add_balance(u["user_id"], total)  # откатываем списание, если товар не начислился
        await message.reply(
...
async def cmd_city_sell(message: Message):
    ...
    price = await aio_get_price(u["city"], item)
    total = price * qty
    await aio_add_balance(u["user_id"], total)
