...
def init_db():
    ...
    with _conn_ctx() as conn:
        ...
        # ── Миграция: колонки first_name/balance/level/xp — дублируют часть
        # data_json прямо в таблице, специально для лидерборда (leaders.py).
        # Раньше _leaders_balance/_leaders_level тянули data_json ВСЕХ
        # пользователей, парсили каждый JSON в Python и сортировали сами —
        # при росте базы это одна из причин лагов (тяжёлый вызов занимал
        # поток из общего пула asyncio.to_thread надолго). Теперь топ-10
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_users_level_xp ON users(level DESC, xp DESC)")
...
def save_user(uid: int, data: dict):
    ...
    Помимо data_json, дублирует username/first_name/balance/level/xp в
...
def transfer_coins(sender_uid: int, recipient_uid: int, amount: int,
    ...
    try:
        ...
        conn.execute(
            ...
            (json.dumps(sender, ensure_ascii=False), sender["balance"], sender_uid)
        )
        conn.execute(
            "UPDATE users SET data_json=?, balance=? WHERE uid=?",
            (json.dumps(recipient, ensure_ascii=False), recipient["balance"], recipient_uid)
        )
        conn.commit()
...
def change_balance(uid: int, delta: int, min_balance: int = 0) -> int | None:
    ...
    try:
        ...
        conn.execute(
            "UPDATE users SET data_json=?, balance=? WHERE uid=?",
            (json.dumps(data, ensure_ascii=False), new_balance, uid)
        )
        conn.commit()
        return new_balance
    except Exception:
...
async def aio_is_charge_processed(charge_id: str) -> bool:
    return await asyncio.to_thread(is_charge_processed, charge_id)


async def aio_mark_charge_processed(charge_id: str, uid: int, payload: str = "") -> None:
    await asyncio.to_thread(mark_charge_processed, charge_id, uid, payload)
