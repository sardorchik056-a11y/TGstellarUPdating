...
async def broadcast_event_start(bot):
    """Рассылает анонс ивента во все известные чаты (личка + группы) и,
    если цикл там ещё не запущен, сразу открывает первый сундук.
    ...
    for chat_id, chat_type in chats:
        ...
        try:
            await bot.send_message(chat_id, event_announce_text(), parse_mode="HTML")
        except Exception:
            continue  # чат недоступен — пропускаем полностью
