...
CARD_REFRESH_SECONDS         = 15        # частота "тихого" авто-обновления карточки
...
def _chats_conn() -> sqlite3.Connection:
    """Лёгкое подключение для горячего пути (вызывается на КАЖДОМ
    апдейте через register_chat). Таблицы уже гарантированно созданы
    при импорте модуля (см. _chats_conn_with_ddl / _init_chats_db_sync),
    поэтому здесь НЕ повторяем CREATE TABLE — это просто connect."""
    return sqlite3.connect(_CHATS_DB_PATH)


def _chats_conn_with_ddl() -> sqlite3.Connection:
    """Полное подключение с DDL. Используется только один раз —
    при инициализации модуля (_init_chats_db_sync)."""
