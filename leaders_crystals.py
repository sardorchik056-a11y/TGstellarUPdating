...
def _fmt(n) -> str:
    """1500 -> '1.5К', 100000 -> '100К', 2300000 -> '2.3М' — единый стиль с city.py
    (короткие заглавные буквы К/М/Б/Т вместо 'к'/'м'/'млрд'/'трлн')."""
    try:
        n = float(n)
...
# ─────────────────────────────────────────
#  АСИНХРОННЫЕ ОБЁРТКИ (использовать из async-хендлеров!)
#  crystal_leaders_text()/crystal_leaders_main_text() внутри дёргают
#  sqlite3 синхронно (_get_conn), поэтому напрямую из event loop их
#  вызывать нельзя — только через asyncio.to_thread.
# ─────────────────────────────────────────

async def aio_crystal_leaders_text(period: str, viewer_uid: int | None = None) -> str:
    return await asyncio.to_thread(crystal_leaders_text, period, viewer_uid)


async def aio_crystal_leaders_main_text(viewer_uid: int | None = None) -> str:
    return await asyncio.to_thread(crystal_leaders_main_text, viewer_uid)
...
async def cmd_crystal_leaders(message: Message):
    ...
    await message.reply(
        await aio_crystal_leaders_main_text(viewer_uid=uid),
        parse_mode="HTML",
        reply_markup=crystal_leaders_main_keyboard(),
    )


def _crystal_check_owner(call: CallbackQuery) -> bool:
    """Проверяет, что кнопку нажимает тот же пользователь, который вызвал
    команду топа кристаллов (сообщение бота было отправлено через .reply()).
...
async def _crystal_deny(call: CallbackQuery):
...
async def cb_crystal_leaders(call: CallbackQuery):
    if not _crystal_check_owner(call):
        await _crystal_deny(call)
        return
    ...
    await call.message.edit_text(
        await aio_crystal_leaders_text(period, viewer_uid=uid),
        parse_mode="HTML",
