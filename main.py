...
from database import format_amount, aio_get_or_create_user, aio_save_user

# Общий per-user лок из mainhelp.py — переиспользуем его же, чтобы ритуалы
# сада не гонялись за баланс/данные с остальными хендлерами (майнер, шоп и т.д.)
from mainhelp import _get_user_lock

# ══════════════════════════════════════════════════════════════════════
#  «САДОВЫЙ ДНЕВНИК» — медитативный раздел без боёв (см. green.py).
#  Вся логика, данные о 80 растениях, ритуалах, достижениях и фоновый
#  цикл уведомлений лежат в green.py; здесь только регистрация команд.
# ══════════════════════════════════════════════════════════════════════
import green as _green


async def _garden_ritual_handler(message: Message, ritual_key: str):
    uid = message.from_user.id
    lock = await _get_user_lock(uid)
    async with lock:
        d = await aio_get_or_create_user(message.from_user)
        res = _green.do_ritual(d, ritual_key)
        await aio_save_user(uid, d)
    await message.reply(_green.ritual_result_text(d, ritual_key, res), parse_mode="HTML")


def _make_ritual_handler(ritual_key: str):
    async def _handler(message: Message):
        await _garden_ritual_handler(message, ritual_key)
    return _handler


# Регистрируем хендлер на каждую команду-ритуал (/полить, /опрыскать, ...)
for _rk in _green.RITUAL_KEYS:
    dp.message(Command(_rk))(_make_ritual_handler(_rk))


@dp.message(Command("сад"))
async def cmd_garden_status(message: Message):
    d = await aio_get_or_create_user(message.from_user)
    _green.get_garden(d)  # прокатывает смену дня, если нужно
    await aio_save_user(message.from_user.id, d)
    await message.reply(_green.garden_status_text(d), parse_mode="HTML")


@dp.message(Command("гербарий"))
async def cmd_herbarium(message: Message):
    d = await aio_get_or_create_user(message.from_user)
    _green.get_garden(d)
    arg = (message.text or "").split(maxsplit=1)
    if len(arg) > 1 and arg[1].strip():
        text = _green.herbarium_category_text(d, arg[1].strip())
    else:
        text = _green.herbarium_overview_text(d)
    await message.reply(text, parse_mode="HTML")


@dp.message(Command("растение"))
async def cmd_plant_info(message: Message):
    d = await aio_get_or_create_user(message.from_user)
    _green.get_garden(d)
    arg = (message.text or "").split(maxsplit=1)
    if len(arg) < 2 or not arg[1].strip():
        await message.reply(
            "Укажи название растения: <code>/растение Роза</code>",
            parse_mode="HTML",
        )
        return
    await message.reply(_green.plant_info_text(d, arg[1].strip()), parse_mode="HTML")


# Оригинальная команда из ТЗ — /достижения, но в mainhelp.py уже занята
# другая команда с тем же именем (общие ачивки бота) — трогать mainhelp.py
# нельзя, поэтому ачивки сада живут на /сад_достижения.
@dp.message(Command("сад_достижения"))
async def cmd_garden_achievements(message: Message):
    d = await aio_get_or_create_user(message.from_user)
    _green.get_garden(d)
    await message.reply(_green.achievements_text(d), parse_mode="HTML")


@dp.message(Command("дневник"))
async def cmd_garden_diary(message: Message):
    d = await aio_get_or_create_user(message.from_user)
    _green.get_garden(d)
    await message.reply(_green.diary_text(d, full=False), parse_mode="HTML")


@dp.message(Command("дневник_полный"))
async def cmd_garden_diary_full(message: Message):
    d = await aio_get_or_create_user(message.from_user)
    _green.get_garden(d)
    text = _green.diary_text(d, full=True)
    # Полная лента может быть длинной — режем по 4000 символов на сообщение
    for i in range(0, len(text), 4000):
        await message.reply(text[i:i + 4000], parse_mode="HTML")


@dp.message(Command("собрать_семя"))
async def cmd_collect_seed(message: Message):
    uid = message.from_user.id
    lock = await _get_user_lock(uid)
    async with lock:
        d = await aio_get_or_create_user(message.from_user)
        g = _green.get_garden(d)
        if g["stage"] < 5:
            await message.reply(
                "🌰 Семена-Наследники появляются только у Древа-Предка (стадия 5).",
                parse_mode="HTML",
            )
            return
        ok = _green.collect_seed(g)
        await aio_save_user(uid, d)
    if ok:
        await message.reply(
            "🌰 Семя-Наследник бережно положено в отдельный карман Гербария. "
            "Оно прорастёт примерно через 3 дня — можно ускорить командой /ускорить_семя (10 🍃).",
            parse_mode="HTML",
        )
    else:
        await message.reply(
            "⏳ Древо ещё не готово дать новое семя — попробуй через несколько дней.",
            parse_mode="HTML",
        )


@dp.message(Command("ускорить_семя"))
async def cmd_speed_seed(message: Message):
    uid = message.from_user.id
    lock = await _get_user_lock(uid)
    async with lock:
        d = await aio_get_or_create_user(message.from_user)
        g = _green.get_garden(d)
        ok = _green.speed_up_seed(g)
        grown = _green.process_ready_seeds(g) if ok else []
        await aio_save_user(uid, d)
    if not ok:
        await message.reply(
            "❌ Либо нет прорастающих семян, либо не хватает 10 🍃 Монет Сада.",
            parse_mode="HTML",
        )
        return
    text = "⚡ Семя ускорено!"
    for p in grown:
        text += "\n\n" + _green.plant_full_description(p)
    await message.reply(text, parse_mode="HTML")


@dp.message(Command("сад_профиль"))
async def cmd_garden_profile(message: Message):
    d = await aio_get_or_create_user(message.from_user)
    _green.get_garden(d)
    await message.reply(_green.garden_profile_text(d), parse_mode="HTML")


@dp.message(Command("итог"))
async def cmd_garden_summary(message: Message):
    d = await aio_get_or_create_user(message.from_user)
    g = _green.get_garden(d)
    await aio_save_user(message.from_user.id, d)
    await message.reply(_green.daily_summary_text(g), parse_mode="HTML")
...
async def _entrypoint():
    ...
    # Фоновый цикл «Садового Дневника»: подсказки-ритуалы, анти-АФК, итог дня.
    asyncio.create_task(_green.green_loop(bot))
    await run_bot()
