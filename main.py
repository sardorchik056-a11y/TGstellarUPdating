...
async def cb_case_guess(call: CallbackQuery, state: FSMContext):
    ...
    if call.message.chat.type != "private":
        ...
        await call.answer(
            "✏️ Чтобы никто не подсмотрел твой ответ, открой диалог со мной "
            "в личке и там нажми эту же кнопку (или пришли /guess число).",
            show_alert=True,
        )
        return
    ...
    await call.message.answer(
        ...
        f"это твоя единственная попытка в этом ивенте.",
        parse_mode="HTML",
    )
    await call.answer()
