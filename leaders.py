...
# ─────────────────────────────────────────
#
# ВАЖНО: раньше здесь везде использовался голый `with sqlite3.connect(...) as
# conn:` / `with _get_conn() as conn:`. У sqlite3.Connection `with` управляет
# ТОЛЬКО транзакцией (commit/rollback) — conn.close() он НЕ вызывает. Ни
# в одном месте этого файла close() не было, поэтому каждый вызов (включая
# record_boss_hit — самую частую операцию в игре, срабатывающую на каждый
# удар по боссу) оставлял за собой незакрытое соединение. Это та же причина
# "database is locked" / зависаний, что уже была найдена и исправлена в
# database.py — здесь фикс просто не был применён. Теперь используем общий
# контекст-менеджер с гарантированным close() в finally, плюс WAL и
# busy_timeout=30000 (как в database.py/hunt.py), которых тут раньше не
# было вовсе (без них соединение ждёт занятую БД лишь ~5 сек по умолчанию
# и куда чаще падает с "database is locked").
...
_PLACE_EMOJI = {
    ...
    4:  "5830434773088083875",
    5:  "5827941630472100575",
    6:  "5830446086031941275",
    7:  "5830341589477629469",
    8:  "5829992769413717353",
    9:  "5830388872772591861",
    10: "5827670648100494160",
}
...
def _leaders_balance(period: str) -> list[dict]:
    """
    Раньше тянула data_json ВСЕХ пользователей, парсила каждый JSON в
    Python и сортировала сама — при росте базы это одна из причин лагов
    (тяжёлый вызов надолго занимал поток из общего пула asyncio.to_thread,
    из-за чего остальные запросы бота вставали в очередь на свободный
    поток). Теперь database.py дублирует balance в отдельную индексированную
    колонку users.balance при каждом save_user()/change_balance()/
    transfer_coins(), поэтому топ-10 достаётся напрямую в SQLite через
...
# ---------- Async-обёртки ----------
# sqlite3 в этом модуле синхронный (блокирующий), как и в database.py/klan.py.
...
# ВСЕ пользователи разом. Особенно критично для record_boss_hit — она
# вызывается на КАЖДЫЙ удар по боссу, самую частую кнопку в игре.
# Использовать эти обёртки из любого async-кода вместо прямого вызова
# синхронных версий выше (*args/**kwargs — чтобы не дублировать сигнатуры).

async def aio_record_boss_hit(*args, **kwargs) -> None:
    await asyncio.to_thread(record_boss_hit, *args, **kwargs)


async def aio_leaders_text(*args, **kwargs) -> str:
    return await asyncio.to_thread(leaders_text, *args, **kwargs)


async def aio_leaders_main_text(*args, **kwargs) -> str:
    return await asyncio.to_thread(leaders_main_text, *args, **kwargs)
