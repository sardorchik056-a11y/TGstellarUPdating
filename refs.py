...
async def aio_register_referral(uid: int, inviter_uid: int | None) -> None:
    await _asyncio.to_thread(register_referral, uid, inviter_uid)


async def aio_is_new_user(uid: int) -> bool:
...
async def aio_get_ref_stats(uid: int) -> dict:
    return await _asyncio.to_thread(get_ref_stats, uid)


async def aio_get_referrals_list(uid: int) -> list[dict]:
    return await _asyncio.to_thread(get_referrals_list, uid)


async def aio_get_inviter(uid: int) -> int | None:
    return await _asyncio.to_thread(get_inviter, uid)


async def aio_get_ref_percent_info(uid: int) -> dict:
    return await _asyncio.to_thread(get_ref_percent_info, uid)


async def aio_refs_main_text(uid: int, bot_username: str, lang: str = "ru") -> str:
    return await _asyncio.to_thread(refs_main_text, uid, bot_username, lang)


async def aio_refs_list_text(uid: int, lang: str = "ru") -> str:
    return await _asyncio.to_thread(refs_list_text, uid, lang)


async def aio_get_reftop(period: str) -> list[dict]:
    return await _asyncio.to_thread(get_reftop, period)


async def aio_reftop_text(period: str, viewer_uid: int | None = None, lang: str = "ru") -> str:
    return await _asyncio.to_thread(reftop_text, period, viewer_uid, lang)
